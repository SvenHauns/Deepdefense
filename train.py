...
def train(train_loader, model, optimizer, weights, calibration_method, classes):
    ...
    loss_criterion = torch.nn.CrossEntropyLoss(weight=weighst)
    correct = 0
    total_loss = 0
    size = 0
    ...
    for batch in train_loader:
        ...
        if calibration_method == "DOC":
            ...
            correct += np.equal(out_come, labels.tolist()).sum().item()
            loss = loss * 1 / classes
    ...
    for batch in val_loader:
        ...
        if calibration_method == "DOC":
            ...
            for i in range(classes):
                ...
                out_eval = torch.tensor(out_eval)

                loss = loss + criterion(out_eval, lab)
...
def test(test_loader, models, calibration_method, classes):
    ...
    for batch in test_loader:
        ...
        for model in models:
            ...
            out.append(out_model.detach().numpy())
